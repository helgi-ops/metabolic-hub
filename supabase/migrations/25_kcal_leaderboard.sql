-- Cumulative kcal leaderboard for the cardio machines (Assault Airbike + Concept2).
-- SECURITY DEFINER so members can see aggregate totals (name + sum) without
-- being able to read each other's individual workout_logs rows. Station-scoped:
-- members/coaches see their own station; admins see all (or a chosen station).
...
revoke all on function public.kcal_leaderboard(uuid, text) from public;
grant execute on function public.kcal_leaderboard(uuid, text) to authenticated;
