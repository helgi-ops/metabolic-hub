...
export default async function StationPage({
  ...
  // Last-active label + at-risk colour per member, from their most recent log.
  const activityMeta = (
    id: string,
  ): { label: string; risk: "none" | "warn" | "at" } => {
    const days = daysSince(lastActive.get(id) ?? null);
    if (days == null) return { label: "Aldrei skráð", risk: "at" };
    if (days === 0) return { label: "Í dag", risk: "none" };
    if (days === 1) return { label: "Í gær", risk: "none" };
    const risk = days >= 14 ? "at" : days >= 7 ? "warn" : "none";
    return { label: `fyrir ${days} dögum`, risk };
  };

  // One roster card. Clicking the name opens a modal with that member's
  // activity (recent logged workouts); the card itself shows their last-active
  // status so at-risk members still stand out at a glance.
  const memberCard = (m: (typeof roster)[number]) => {
    const { label, risk } = activityMeta(m.id);
    return (
      <MemberCard
        key={m.id}
        member={{
